...
export function getCategoryKey(categorySlug = '') {
...
export function deriveSearchQueryFromTitle(title = '', catKey = 'civel') {
  ...
  if (normalized.includes('dip') || normalized.includes('financiamento') || normalized.includes('debenture')) {
    terms.push('corporate finance liquidity loan business');
  } else if (normalized.includes('recuperacao') || normalized.includes('falencia') || normalized.includes('reestruturacao') || normalized.includes('concordata') || normalized.includes('crise') || normalized.includes('passivo')) {
    terms.push('corporate restructuring finance negotiation boardroom');
  }

  if (normalized.includes('agro') || normalized.includes('soja') || normalized.includes('safra') || normalized.includes('cpr') || normalized.includes('rural') || normalized.includes('fazenda') || normalized.includes('grao') || normalized.includes('lavoura')) {
    terms.push('modern agriculture harvest grain silo agribusiness farm');
  }

  if (normalized.includes('tributar') || normalized.includes('fiscal') || normalized.includes('imposto') || normalized.includes('reforma tributaria') || normalized.includes('credito tributario') || normalized.includes('icms') || normalized.includes('pis') || normalized.includes('cofins')) {
    terms.push('tax corporate finance accounting fiscal audit document');
  }

  if (normalized.includes('holding') || normalized.includes('sucess') || normalized.includes('heranc') || normalized.includes('patrimon') || normalized.includes('familia') || normalized.includes('inventario') || normalized.includes('partilha') || normalized.includes('divorcio')) {
    terms.push('wealth management family office finance estate planning');
  }

  if (normalized.includes('societari') || normalized.includes('socio') || normalized.includes('fusao') || normalized.includes('aquisicao') || normalized.includes('m&a') || normalized.includes('governanca') || normalized.includes('acionista')) {
    terms.push('merger acquisition corporate boardroom business partners meeting');
  }

  if (normalized.includes('contrat') || normalized.includes('clausul') || normalized.includes('rescis') || normalized.includes('acordo') || normalized.includes('pacto')) {
    terms.push('business contract signing fountain pen documents agreement');
  }

  if (normalized.includes('imobiliari') || normalized.includes('imovel') || normalized.includes('locacao') || normalized.includes('leilao') || normalized.includes('incorporac') || normalized.includes('construc') || normalized.includes('terreno')) {
    terms.push('commercial real estate modern architecture skyscraper property');
  }

  if (normalized.includes('medic') || normalized.includes('saude') || normalized.includes('hospital') || normalized.includes('clinica') || normalized.includes('cirurgi') || normalized.includes('paciente') || normalized.includes('crm')) {
    terms.push('hospital doctor healthcare facility medical consultation clinic');
  }

  if (normalized.includes('trabalh') || normalized.includes('empreg') || normalized.includes('rescisao') || normalized.includes('clt') || normalized.includes('terceiriz') || normalized.includes('salario') || normalized.includes('hora extra')) {
    terms.push('corporate human resources office workplace employment negotiation');
  }

  if (normalized.includes('tecnologi') || normalized.includes('lgpd') || normalized.includes('dado') || normalized.includes('inteligencia artificial') || normalized.includes('software') || normalized.includes('seguranca da informacao')) {
    terms.push('modern technology cybersecurity data servers corporate business');
  }

  if (normalized.includes('bancari') || normalized.includes('banco') || normalized.includes('juros') || normalized.includes('divida') || normalized.includes('execucao') || normalized.includes('cobranca') || normalized.includes('alienacao')) {
    terms.push('banking financial district corporate negotiation investment');
  }

  if (normalized.includes('ambiental') || normalized.includes('esg') || normalized.includes('sustentab') || normalized.includes('licenciamento')) {
    terms.push('corporate sustainability green energy esg environmental business');
  }

  if (normalized.includes('compliance') || normalized.includes('anticorrupcao') || normalized.includes('auditoria') || normalized.includes('risco')) {
    terms.push('corporate compliance governance business meeting audit');
  }
  ...
  const stopWords = new Set([
    'para', 'com', 'sem', 'sob', 'sobre', 'como', 'onde', 'quando', 'quem', 'qual',
    'mais', 'menos', 'entre', 'apos', 'antes', 'pelo', 'pela', 'pelos', 'pelas',
    'esse', 'essa', 'esses', 'essas', 'este', 'esta', 'estes', 'estas', 'aquele',
    'aquela', 'aqueles', 'aquelas', 'isso', 'isto', 'aquilo', 'voce', 'voces',
    'artigo', 'caso', 'guia', 'completo', 'entenda', 'saiba', 'veja', 'passo', 'dicas',
    'novo', 'nova', 'novos', 'novas', 'tudo', 'sobre', 'quais', 'seus', 'suas', 'qual'
  ]);
  ...
  if (cleanWords.length > 0) {
    const sampleWords = cleanWords.slice(0, 2).join(' ');
...
export async function fetchTopicImages(arg1 = '', arg2 = [], arg3 = {}) {
  ...
  if (unsplashKey) {
    try {
      // 1. Prioriza o título do artigo para montar a query de busca contextualizada
      const titleDerivedQuery = deriveSearchQueryFromTitle(title, catKey);
      const pool = SEARCH_QUERY_POOLS[catKey] || SEARCH_QUERY_POOLS.recuperacao;
      let chosenQuery = titleDerivedQuery || pool[Math.floor(Math.random() * pool.length)];

      // Se houver palavras-chave contextuais do artigo e não derivou do título, intercala para maior relevância
      if (!titleDerivedQuery && keywords && keywords.length > 0 && Math.random() > 0.4) {
  ...
  return getCuratedImages(categorySlug, excludeUrls, title);
}

/**
 * Retorna imagens do acervo curado com sorteio aleatório e verificação de repetição
 */
export function getCuratedImages(categorySlug = '', excludeUrls = [], title = '') {
  let catKey = getCategoryKey(categorySlug);

  // Se o título indicar uma área temática jurídica mais específica que o categorySlug genérico:
  if (title) {
    const titleCat = getCategoryKey(title);
    if (titleCat && titleCat !== 'civel') {
      catKey = titleCat;
    }
  }

  const baseList = CURATED_IMAGES[catKey] || CURATED_IMAGES.civel || CURATED_IMAGES.recuperacao;
  const excludedIds = new Set(excludeUrls.map(extractPhotoId).filter(Boolean));
  ...
  console.log(
    `[ImageProvider] Curadoria dinâmica selecionou fotos para "${catKey}" (título: "${title}"). Capa ID: ${extractPhotoId(
      finalPool[0].url
    )}`
  );
